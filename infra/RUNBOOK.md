...
このデプロイは「アプリが本番で使える＝完成」を意味する。よってここでの確認は**インフラ起動のスモークで終わらせず、ブラウザで全機能を実操作する受け入れ試験**まで行う。

> **前提**: 現 `main` から**両方のイメージを再ビルド & push 済み**であること（§2.5）。古いイメージのままだと
> 新しい機能（下記 A 群）がコンテナに入っておらず、試験が無意味になる。

#### 2.7.1 スモーク（まずタスクが上がったか）
...
#### 2.7.2 受け入れ試験（ブラウザで `http://<alb_dns_name>/` を開き、1 機能ずつ実操作）

**A. 必ず再検証**（#157 のデプロイ E2E より後に追加され、実 AWS で一度も動かしていない／AWS で挙動が変わる経路）

| # | 機能 | 操作と合格条件 |
| --- | --- | --- |
| A1 | ワークスペース作成 | 作成 → general 自動作成 → 作成した WS に遷移 |
| A2 | DM 新規作成 | メンバー選択 → ルーム作成 → メッセージ送受信 |
| A3 | 絵文字ピッカー | メッセージにリアクション付与 → カウント反映 |
| A4 | **ファイル添付送信** | 画像添付して送信 → **実 S3 にアップロードされ公開 URL で画像表示** |
| A5 | チャンネル作成 | public / private を各 1 つ作成 |
| A6 | **メンバー招待** | 2 セッション: A が招待 → **B のサイドバーに当該 ch が出現** |
| A7 | **メンバーキック** | 2 セッション: A が削除 → **B からその ch がリロード無しで消える** |
| A8 | **CHANNEL_ADDED 通知** | 2 セッション: A が B を追加 → **B にバナー＋サイドバー即時反映＋ヘッダのメンバー数バッジが実数** |
| A9 | **file-only 添付** | 本文を空にして画像のみ送信 → S3 アップロード → **本文バブル無しで添付のみ描画** |

**B. リグレッション**（#157 で実 AWS ✅ 済み・軽く再確認）

- 認証: signup / login / refresh（JWT は Secrets Manager・セッション維持）
- メッセージ: 送信 / 履歴 / 編集 / 削除
- WebSocket: 2 タブでリアルタイム送受信（Redis Pub/Sub）
- アバター画像アップロード（S3・公開配信）
- ALB ルーティング: `/`（フロント）・`/api/*`（REST）・`/ws/*`（WebSocket）

**C. インフラ・移行の確証**（上記を通すと間接的に取れる）

- **V3 マイグレーションが RDS で適用済み** … アプリが起動した時点で Flyway 成功。確証が欲しければ
  §5 の bastion から `psql` で `\d messages` を見て `messages_body_length` が `char_length(body) <= 4000` になっているか確認。
- **ElastiCache の Pub/Sub がタスク跨ぎで動く** … `desired_count=2`（Multi-AZ）のため A6〜A8 の 2 セッションは
  別タスクに着地し得る。それでもリアルタイム反映されれば**分散環境の Redis fan-out が成立**している証拠
  （ローカル単一インスタンスでは検出できない、AWS でしか確かめられない価値）。
- **destroy 後の残存ゼロ** … §6.2 で確認（課金漏れ防止）。

> A6〜A8 の「2 セッション」は、別ブラウザ / シークレットウィンドウ / 別端末で 2 ユーザーを同時ログインさせて行う。
> 受け入れ試験は chrome-devtools 等でこの画面操作を自動化して回しきってもよい。
