...
| インフラ・デプロイ構成（AWS）| ⛔ 本書スコープ外（[インフラ設計書](infrastructure.md) で確定済み）|
...
| フロント配信 | ✅ ECS Fargate でホスト（Next.js `output: 'standalone'`）＋ 単一 ALB パスルーティング。`default`→フロント / `/api`・`/ws`→バックエンドで同一オリジン配信（[infrastructure.md §12.2](infrastructure.md)）|
| IaC | ✅ Terraform（[`infra/terraform/`](../infra/terraform/README.md)。state は S3 + DynamoDB / bootstrap で管理）|
| 冗長化構成（フロント・バックエンド複数台）| ✅ フロント / バックエンドとも ECS Fargate 複数タスク（Multi-AZ）+ Redis Pub-Sub（[infrastructure.md §6](infrastructure.md) / [realtime-design.md](realtime-design.md)）|
| 自動デプロイ / CI/CD | ✅ GitHub Actions → ECR → ECS（フロント・バックエンドとも。方針確定。[infrastructure.md §7](infrastructure.md)）|
